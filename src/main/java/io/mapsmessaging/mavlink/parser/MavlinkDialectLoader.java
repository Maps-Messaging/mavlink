...
public class MavlinkDialectLoader {
  ...
  private void mergeInto(MavlinkDialectDefinition target, MavlinkDialectDefinition source) {
    ...
    if (source.getEnumsByName() != null) {
      for (Map.Entry<String, MavlinkEnumDefinition> e : source.getEnumsByName().entrySet()) {
        if (e.getKey() == null) {
          continue;
        }
    ...
    if (source.getMessagesById() != null) {
      for (Map.Entry<Integer, MavlinkMessageDefinition> e : source.getMessagesById().entrySet()) {
  ...
  private void normalize(MavlinkDialectDefinition def) {
    ...
    List<MavlinkMessageDefinition> messages = new ArrayList<>(ids.size());
    for (Integer id : ids) {
